...
taxonomy <- taxonomy %>% mutate_all(na_if,"") %>% filter(., L6 != "NA")


otu_with_taxa <- t(otu_with_taxa)
otu_with_taxa <- merge(taxonomy, otu_with_taxa, by.x = "Feature.ID", by.y = "row.names")
otu_with_taxa <- otu_with_taxa %>% 
  select(., L6, 10:158) %>%
  group_by(., L6) %>%
  summarise(across(where(is.numeric), ~ sum(.x, na.rm = TRUE))) %>%
  remove_rownames(.) %>%
  column_to_rownames(., var = "L6") %>%
  t()
